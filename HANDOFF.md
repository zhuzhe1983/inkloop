...
当前支持天气、海报/图片、时钟、专注卡、倒计时、指标卡、月历、课程表和多日智能日程。智能日程可读取多个 iCal / webcal 数据源，支持横竖屏、三日/五日时间轴、重叠日程并排和空闲时段压缩。
...
2. 定时任务依赖前台页面，无法保证系统级长期调度。若要脱离电脑浏览器，需要原生应用、常驻本地服务或硬件端网络能力，不能只靠网页定时器解决。
