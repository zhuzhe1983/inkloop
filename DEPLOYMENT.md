...
| `BAIDU_MAP_BASE_URL` | Docker 必需 | Docker 内设为 `http://llm-proxy:8788/baidu`，Sites 留空直连百度 |
| `OUTBOUND_PROXY_BASE_URL` | Docker 必需 | Docker 内设为 `http://llm-proxy:8788/outbound`，Sites 留空直连图片和天气上游 |
| `LLM_PROXY_UPSTREAM` | Docker 可选 | Docker 私有代理的 HTTPS 上游，默认 `https://hub.tsingfly.com` |
| `BAIDU_MAP_UPSTREAM` | Docker 可选 | Docker 私有代理的百度 HTTPS 上游，默认 `https://api.map.baidu.com` |
...
Docker 中的 `llm-proxy` 只暴露在 Compose 私有网络，不映射宿主机端口。它使用 Node 系统 CA 校验并按白名单转发 LLM、百度地图、图片素材和天气 HTTPS 请求，解决本地 Workers 运行时与部分上游证书链的兼容差异；API Key 仍由 Inkloop Worker 在请求时传入，不保存在代理镜像或日志中。
