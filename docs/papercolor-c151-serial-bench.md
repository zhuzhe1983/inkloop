...
Machine-verifiable checks require the exact non-empty startup chain `RESET_REASON:0..10` → `BOOT:<firmware>` → `BOARD:28` → `PM1:READY` → `HARDWARE_READY:READY`. Values follow the pinned ESP-IDF enum exactly: `0 UNKNOWN`, `1 POWERON`, `2 EXT`, `3 SW`, `4 PANIC`, `5 INT_WDT`, `6 TASK_WDT`, `7 WDT`, `8 DEEPSLEEP`, `9 BROWNOUT`, and `10 SDIO`; values `11` and above are rejected until the pinned SDK deliberately adds them. Initial hard-reset value `0` is preserved as typed `UNKNOWN` evidence and emits a report warning—it is never relabeled as `POWERON`. A matching device command echo and status/diagnostic object must arrive after the successfully written command, in the same fresh receive epoch. The response must contain a non-empty firmware string of at most 64 characters plus a valid `M5PC-XXXXXXXXXXXX` hardware ID, and both must equal the earlier boot/hardware identities. Wi-Fi AP/connected state, a single shared six-digit MyAI/Inkloop code when emitted, blocked activation/payment states, runtime errors, optional test acknowledgements, and credential hygiene are also checked. If firmware emits `REBOOTING`, the later reset must be the controlled software-reset value `3`, followed by a complete startup chain.
