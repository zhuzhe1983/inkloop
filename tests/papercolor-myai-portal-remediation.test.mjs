...
int main() {
  ...
  assert(portalIdentityMatchesSnapshot(
      PortalIdentityState::BoundRecovery, true, false, false, false));
  assert(!portalIdentityMatchesSnapshot(
      PortalIdentityState::BoundRecovery, false, false, false, false));
  assert(!portalIdentityMatchesSnapshot(
...
test("concrete Portal/MyAI adapters enforce typed recovery, terminal scrub, and pre-allocation frame checks", async () => {
  ...
  assert.match(application, /mac\[5\], mac\[4\], mac\[3\], mac\[2\], mac\[1\], mac\[0\]/);
