...
struct FakeHttp final : IHttpTransport {
  ...
  Status perform(const HttpRequest& request, HttpResponse& response) override {
    ...
    if (request.url.find("/healthz") != std::string::npos) {
      ...
      response.body = "{\"device_id\":\"654321\",\"app_id\":\"inkloop\",\"status\":\"claimed\",\"bound\":true,\"device_token\":\"private-device-token\",\"device\":{\"active\":true},\"expires_at\":\"2026-08-22T01:00:00Z\"}";
...
int main() {
  ...
  config.macAddress = "0C:DA:43:85:84:28";
  ...
  assert(http.requests.back().body.find("\"mac_address\":\"0C:DA:43:85:84:28\"") != std::string::npos);
  ...
  assert(ws.url.find("mac_address=0C%3ADA%3A43%3A85%3A84%3A28") != std::string::npos);
  ...
  for (size_t index = 0; index < http.requests.size(); ++index) {
    ...
    if (deviceMac != request.headers.end()) {
      assert(deviceMac->second == "0C:DA:43:85:84:28");
  ...
  assert(errorStore.value.deviceToken.empty() && errorStore.runtimeClears == 1);
  return 0;
}
`;
