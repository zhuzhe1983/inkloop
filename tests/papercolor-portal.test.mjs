...
int main() {
  ...
  assert(portal.handle(request(
      "POST", "/api/session", 5, "nonce=boot-nonce-1234567890")).status == 409);
  ...
  const PortalResponse saved = authenticated(
      ...
      "/api/settings",
      32,
      "storage=sd&volume=75&led_brightness=80&assistant_prompt=Draw+calm+art&image_size=600x400&"
  ...
  const std::string dashboard = portal.renderDashboardHtml();
  ...
  assert(dashboard.size() <= kMaximumDashboardHtmlBytes);
  ...
  assert(inactivePortal.renderDashboardHtml().find(">808080<") == std::string::npos);
