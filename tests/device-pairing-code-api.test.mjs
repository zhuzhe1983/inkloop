...
test("formal migration succeeds only after the read-only duplicate preflight passes", async (t) => {
  assert.equal(migrations.at(-1).name, "0005_wide_lionheart.sql");
  assert.match(migrations.at(-1).sql, /CREATE UNIQUE INDEX `idx_devices_pairing_code_unique`/);
  assert.match(migrations.at(-1).sql, /WHERE .*pairing_code.* IS NOT NULL/);
  ...
  const worker = await createWorker({ migrationCount: migrations.length - 1 });
  t.after(() => worker.dispose());
  const db = await worker.getD1Database("DB");
  ...
  await applyMigration(db, migrations.at(-1));
  const uniqueIndex = await db.prepare(`SELECT sql FROM sqlite_master
    WHERE type = 'index' AND name = 'idx_devices_pairing_code_unique'`).first();
  assert.match(uniqueIndex.sql, /CREATE UNIQUE INDEX/i);
...
test("legacy duplicate preflight blocks migration without changing or disclosing rows", async (t) => {
  ...
  await assert.rejects(() => applyMigration(db, migrations.at(-1)));
