...
const firmwareSource = new URL(
  "../firmware/m5-papercolor/src/",
  import.meta.url,
);
...
int main() {
  PagePromptPlan fourth = pageOrdinalPromptPlan(4, false);
  assert(fourth.count == 3);
  assert(std::strcmp(fourth.prompts[0], "ordinal.prefix") == 0);
  assert(std::strcmp(fourth.prompts[1], "ordinal.digit_four") == 0);
  assert(std::strcmp(fourth.prompts[2], "ordinal.suffix") == 0);
  PagePromptPlan tenth = pageOrdinalPromptPlan(10, true);
  assert(tenth.count == 4);
  assert(std::strcmp(tenth.prompts[0], "display.refresh_start") == 0);
  assert(std::strcmp(tenth.prompts[2], "ordinal.ten") == 0);
  PagePromptPlan ninetySixth = pageOrdinalPromptPlan(96, false);
  assert(ninetySixth.count == 5);
  assert(std::strcmp(ninetySixth.prompts[1], "ordinal.digit_nine") == 0);
  assert(std::strcmp(ninetySixth.prompts[2], "ordinal.ten") == 0);
  assert(std::strcmp(ninetySixth.prompts[3], "ordinal.digit_six") == 0);
...
test("physical and voice page queues share the pending lifecycle seam", async () => {
  ...
  assert.match(main, /kPageSelectionSettleMs\s*=\s*1000/);
  assert.match(buttonBody, /notifyPageSelected\(selection\.page \+ 1\)/);
  assert.match(processBody, /millis\(\) - pendingPageAt < kPageSelectionSettleMs/);
  assert.match(processBody, /notifyPageRefreshStarting\(target \+ 1\)/);
