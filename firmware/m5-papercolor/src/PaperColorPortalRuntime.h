...
class IPaperColorPortalServices {
 public:
  ...
  virtual bool restartMyAiPairing(std::string* error) = 0;
  ...
  virtual bool setAlbumRenderStrategy(
      const std::string& assetId,
  ...
  virtual bool openAlbumPreview(
      const std::string& assetId, File* file, size_t* bytes,
      std::string* error) {
    (void)assetId;
    (void)file;
    (void)bytes;
    if (error) *error = "album_preview_unavailable";
    return false;
  }
...
class PaperColorPortalRuntime final : public portal::IPortalAdapter {
 public:
  ...
  bool restartMyAiPairing(const char* appId, std::string* error) override;
 ...
 private:
  ...
  void handleAlbumPreviewRequest();
