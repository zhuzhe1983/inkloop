...
bool PaperColorPortalRuntime::restartMyAiPairing(
    const char* appId, std::string* error) {
  if (!appId || std::string(appId) != "inkloop") {
    if (error) *error = "invalid_myai_app_id";
    return false;
  }
  return services_.restartMyAiPairing(error);
}
...
bool PaperColorPortalRuntime::setAlbumRenderStrategy(
    const std::string& assetId,
    const std::string& renderStrategy,
    std::string* error) {
  return services_.setAlbumRenderStrategy(assetId, renderStrategy, error);
}
...
bool PaperColorPortalRuntime::encodeSnapshot(
  ...
  settings["voice_assistance"] = snapshot.settings.voiceAssistanceEnabled;
...
bool PaperColorPortalRuntime::decodeSnapshot(
  ...
  if (!safeEnum(stage, static_cast<uint8_t>(portal::OnboardingStage::SettingsReady)) ||
      ...
      !safeEnum(refresh, static_cast<uint8_t>(portal::RefreshMode::SolidClean)) ||
  ...
  // Early schema-v2 snapshots predate this preference. Voice assistance was
  // the old behavior, so absence migrates safely to enabled on the next save.
  decoded.settings.voiceAssistanceEnabled =
      settings["voice_assistance"] | true;
...
void PaperColorPortalRuntime::handleAlbumPreviewRequest() {
  ...
  const portal::PortalResponse authorization =
      portal_->authorizeStreamingAlbumPreview(
          requestFromServer("/api/album/preview"));
  if (authorization.status != 200) {
    if (authorization.retryAfterSeconds) {
      server_.sendHeader(
          "Retry-After", String(authorization.retryAfterSeconds));
    }
    server_.send(
        authorization.status, authorization.contentType.c_str(),
        authorization.body.c_str());
    requestActive_ = false;
    return;
  }
  const std::string assetId = server_.arg("asset_id").c_str();
  if (!server_.hasArg("asset_id") || server_.args() != 1 ||
      !portal::isSafeAssetId(assetId)) {
    server_.send(
        400, "application/json",
        "{\"ok\":false,\"error\":\"invalid_asset_target\"}");
    requestActive_ = false;
    return;
  }
  File file;
  size_t bytes = 0;
  std::string error;
  if (!services_.openAlbumPreview(assetId, &file, &bytes, &error) ||
      !file || bytes < 45 || bytes > portal::kMaximumAlbumUploadBytes ||
      file.size() != bytes) {
    if (file) file.close();
    const int status = error == "device_busy" ? 409 : 404;
    server_.send(
        status, "application/json",
        (std::string("{\"ok\":false,\"error\":\"") +
         portal::jsonEscape(error.empty() ? "album_item_not_found" : error) +
         "\"}").c_str());
    requestActive_ = false;
    return;
  }
