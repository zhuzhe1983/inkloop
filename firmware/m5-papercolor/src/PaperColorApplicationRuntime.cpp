...
bool PaperColorApplicationRuntime::begin(bool wifiConfigured) {
  ...
  portalMyAiState_ = activationName(activation);
  ...
  if (!wakeRecovery_.beginAfterHardwareReady(
          Esp32DevicePowerAdapter::detectedWakeReason())) {
...
bool PaperColorApplicationRuntime::activateDisplayOwner() {
  ...
  config.enabled = true;
  config.experimentalPrequantizationEnabled =
      settingsStore_.current().features.experimentalRenderEnabled ||
      portal_.settings().refreshMode == portal::RefreshMode::ExperimentalSixColor;
...
bool PaperColorApplicationRuntime::refreshFrame(
  ...
  request.strategy = portal_.settings().refreshMode ==
                             portal::RefreshMode::ExperimentalSixColor
      ? displaypower::RenderStrategy::ExperimentalSixColor
      : displaypower::RenderStrategy::OfficialQuality;
...
void PaperColorApplicationRuntime::onPageDisplayCommitted(
  ...
  // Physical page keys do not create a voice selection ticket, but this call
  // itself occurs only after the exact display transaction committed.
  if (success) {
    promptPlayer_.play(
        voice::AudioPromptController::ordinalFileId(oneBasedOrdinal),
        oneBasedOrdinal > 3 ? oneBasedOrdinal : 0);
  }
}

void PaperColorApplicationRuntime::notifyPageBusy() {
  if (!promptPlayer_.busy() && !M5.Mic.isRecording()) {
    promptPlayer_.play("display.please_wait");
  }
}
...
portal::AlbumReadStatus PaperColorApplicationRuntime::readAlbumPage(
  ...
  for (const AlbumCatalogEntry& source : entries) {
    ...
    item.title = source.taskId.length() ? source.taskId.c_str() : "Inkloop image";
    item.origin = source.taskId.startsWith("myai:") ? "myai" : "inkloop";
...
portal::AlbumReadStatus PaperColorApplicationRuntime::findAlbumItem(
  ...
  item->id = source.id.c_str();
  item->title = source.taskId.c_str();
  item->origin = source.taskId.startsWith("myai:") ? "myai" : "inkloop";
  item->bytes = source.bytes;
  item->current = source.current;
  item->factoryAsset = source.factoryAsset;
...
portal::DiagnosticsSnapshot PaperColorApplicationRuntime::portalDiagnostics() const {
  ...
  snapshot.displayState = displayBusy() ? "busy" : "ready";
...
void PaperColorApplicationRuntime::pollPairing() {
  ...
  if (!status.ok()) {
    if (status.code == myai::ErrorCode::PairingExpired ||
      ...
      if (status.code != myai::ErrorCode::RecoveryRequired) {
...
void PaperColorApplicationRuntime::pollAigc() {
  ...
  if (aigcPhase_ == AigcPhase::Start) {
    ...
    const bool shown = loaded && refreshFrame(asset.id, frame.bytes, frame.length, false);
    frame.release();
    if (shown) {
