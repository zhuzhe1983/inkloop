...
class PaperColorApplicationRuntime final
 ...
 public:
  ...
  bool refreshFrame(
      ...
      const String& requestedRenderStrategy = String());
  bool displayBusy() const;
  void onPageDisplayCommitted(
      const String& assetId, size_t oneBasedOrdinal, bool success);
  void notifyPageSelected(size_t oneBasedOrdinal);
  void notifyPageRefreshStarting(size_t oneBasedOrdinal);
  void notifyPageBusy();
  ...
  bool voiceAssistanceEnabled() const;
  ...
  bool restartMyAiPairing(std::string* error) override;
 ...
 private:
  ...
  bool playAssistancePrompt(const char* promptId, uint32_t argument = 0);
  void clearAssistancePromptQueue(bool stopCurrent);
  ...
  uint32_t wakeIndicatorOffAt_ = 0;
  displaypower::WakeReason wakeReason_ = displaypower::WakeReason::ColdBoot;
  static const uint8_t kAssistancePromptQueueCapacity = 8;
  const char* assistancePromptQueue_[kAssistancePromptQueueCapacity] = {};
  uint8_t assistancePromptQueueHead_ = 0;
  uint8_t assistancePromptQueueCount_ = 0;
