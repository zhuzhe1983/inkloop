...
const char* refreshModeName(RefreshMode mode) {
  return mode == RefreshMode::ExperimentalSixColor
      ? "experimental-six-color" : "official";
}
...
const char* myAiStateMessage(const std::string& state, bool hasCode) {
  ...
  if (state == "inactive") return "MyAI 已绑定，但当前需要激活或恢复服务。";
...
std::string InkloopPortal::normalizedRateRoute(const std::string& path) const {
  static const char* routes[] = {
      ...
      "/api/album/upload", "/api/album/display", "/api/aigc/generate",
      "/api/audio/preview",
      "/api/onboarding/myai/start", "/api/tutorial/advance",
...
PortalResponse InkloopPortal::handleSession(const PortalRequest& request) {
  ...
  if (sessionIssued_) return errorResponse(409, "bootstrap_nonce_already_used");
  std::map<std::string, std::string> fields;
...
PortalResponse InkloopPortal::handleSettings(
  ...
  static const char* allowed[] = {
      "storage", "volume", "led_brightness", "assistant_prompt", "image_size", "image_steps",
  ...
  if (field != fields.end()) {
    if (field->second == "official") next.refreshMode = RefreshMode::OfficialQuality;
    else if (field->second == "experimental-six-color") {
...
AlbumReadStatus InkloopPortal::readValidatedAlbumPage(
  ...
  for (size_t index = 0; index < page->items.size(); ++index) {
    ...
    const size_t itemBytes = item.id.size() + item.title.size() + item.origin.size();
...
PortalResponse InkloopPortal::renderDashboardResponse() const {
  ...
  prefix << "<!doctype html><html lang=\"zh-CN\"><head><meta charset=\"utf-8\">"
         ...
         << "<section><h2>初始化</h2>"
         << "<p>状态：" << htmlEscape(onboardingStageName(onboarding_.stage())) << "</p>"
  ...
  prefix << "<form id=\"album-upload-form\" data-width=\""
         ...
         << "<button id=\"album-upload-button\" type=\"submit\" disabled>确认并上传</button></form><ul>";
  std::string output;
  ...
  for (size_t index = 0; index < page.items.size(); ++index) {
    std::ostringstream item;
    item << "<li data-asset-id=\"" << htmlEscape(page.items[index].id) << "\">"
         << htmlEscape(page.items[index].title) << " · "
         << page.items[index].bytes << " bytes";
    if (isSafeAssetId(page.items[index].id)) {
      item << "<form class=\"inline\" data-portal=\"1\" method=\"post\" "
    ...
    if (!page.items[index].factoryAsset && isSafeAssetId(page.items[index].id)) {
      item << "<form class=\"inline\" data-portal=\"1\" data-destructive=\"1\" "
             ...
             << "\"><button class=\"danger\" type=\"submit\">删除（需顶部键确认）</button></form>";
    }
    item << "</li>";
  ...
  suffix << "<form data-portal=\"1\" data-destructive=\"1\" method=\"post\" action=\"/api/actions/prepare\">"
         ...
         << "<input type=\"hidden\" name=\"_csrf\" value=\"" << htmlEscape(access_.csrfToken)
         << "\"><label>刷新策略 <select name=\"refresh_mode\"><option value=\"official\""
         << (settings_.refreshMode == RefreshMode::OfficialQuality ? " selected" : "")
         << ">官方画质</option><option value=\"experimental-six-color\""
         << (settings_.refreshMode == RefreshMode::ExperimentalSixColor ? " selected" : "")
         << ">六色预量化（实验）</option></select></label>"
         ...
         << "</section></div></div></main><script>" << kDashboardScript
         << "</script><script>" << kLedBrightnessScript
         << "</script><script>" << kSettingsScript
         << "</script><script>" << kAlbumUploadScript
         << "</script></body></html>";
