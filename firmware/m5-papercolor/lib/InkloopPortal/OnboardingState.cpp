...
bool OnboardingState::requestMyAiRebind(
    IPortalAdapter& adapter, std::string* error) {
  if (!wifiConfigured_ || !inkloopBound_ ||
      (stage_ != OnboardingStage::MyAiInactive &&
       stage_ != OnboardingStage::VoiceTutorial &&
       stage_ != OnboardingStage::SettingsReady)) {
    setError("myai_rebind_not_available", error);
    return false;
  }
  std::string adapterError;
  if (!adapter.restartMyAiPairing(kMyAiAppId, &adapterError)) {
    if (error) {
      *error = adapterError.empty()
          ? "myai_rebind_not_started" : adapterError;
    }
    return false;
  }
  myAiActive_ = false;
