...
class IPortalAdapter {
 public:
  ...
  virtual bool restartMyAiPairing(const char* appId, std::string* error) {
    (void)appId;
    if (error) *error = "myai_rebind_unavailable";
    return false;
  }
  ...
  virtual bool setAlbumRenderStrategy(
    ...
    (void)assetId;
    (void)renderStrategy;
    if (error) *error = "album_render_strategy_unavailable";
    return false;
  }
