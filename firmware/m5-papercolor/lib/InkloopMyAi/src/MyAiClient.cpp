...
MyAiClient::MyAiClient(
    ...
    : config_(config), http_(http), webSocket_(webSocket), output_(output),
...
Status MyAiClient::pollPairing(bool& bound) {
  ...
  if (parsed.paymentRequired) {
    status = Status(ErrorCode::PaymentRequired, 402, "MyAI activation/payment required");
    setActivation(ActivationState::PaymentRequired, status);
    return status;
  }
  if (!parsed.bound) return Status::success();
...
Status MyAiClient::openGatewaySession(Capability capability,
  ...
  status = perform(centerRequest(
                       "GET", std::string("/api/v1/model-preferences?app_id=") +
                                  urlEncode(kAppId),
                       "", true),
                   preferenceResponse, RequestKind::CenterAuthenticated);
  if (!status.ok()) return status;
  std::string providerProfileId;
  status = codec_.parseModelPreference(preferenceResponse.body, providerProfileId);
  if (!status.ok()) return status;
  if (!nonEmptyOpaqueValue(providerProfileId, 512))
    return Status(ErrorCode::Protocol, 0, "invalid MyAI provider profile id");
  ...
  size_t fastest = requested.gateways.size();
  uint32_t fastestLatency = UINT32_MAX;
  for (size_t index = 0; index < requested.gateways.size(); ++index) {
    GatewayProbe probe;
    probe.gatewayId = requested.gateways[index].id;
    probe.checkedAt = clock_.utcIso8601();
    if (!nonEmptyOpaqueValue(requested.gateways[index].id, 256) ||
        !isPublicGatewayUrl(requested.gateways[index].pingUrl) ||
        !isPublicGatewayUrl(requested.gateways[index].baseUrl)) {
      probe.error = "non-public gateway URL rejected";
      probes.push_back(probe);
      continue;
    }
    HttpRequest request;
    request.method = "HEAD";
    request.url = requested.gateways[index].pingUrl;
    request.headers = deviceHeaders(false);
    request.timeoutMs = 8000;
    request.maxResponseBytes = 0;
    HttpResponse response;
    const uint64_t started = clock_.monotonicMs();
    Status probeStatus = endpointSecurity_.validatePublicTlsEndpoint(request.url);
    if (probeStatus.ok()) probeStatus = http_.perform(request, response);
    const uint64_t elapsed = clock_.monotonicMs() - started;
    probe.latencyMs = static_cast<uint32_t>(std::min<uint64_t>(elapsed, UINT32_MAX));
    probe.ok = probeStatus.ok() && response.status >= 200 && response.status < 400;
    if (!probe.ok) probe.error = probeStatus.ok() ? "gateway probe HTTP failure" : "gateway probe transport failure";
    if (probe.ok && probe.latencyMs < fastestLatency) {
      fastestLatency = probe.latencyMs;
      fastest = index;
    }
    probes.push_back(probe);
  }
  if (fastest == requested.gateways.size())
    return Status(ErrorCode::NoGateway, 503, "no reachable public MyAI gateway",
                  reconnectDelayMs(0));

  const GatewayCandidate& selectedCandidate = requested.gateways[fastest];
  ...
  if (!lease.valid() || lease.providerProfileId.empty()) {
    lease.clearSensitive();
    lease = GatewayLease();
...
Status MyAiClient::classifyHttp(const HttpResponse& response,
  ...
  Status status;
  if (httpStatus == 401) {
    status = Status(ErrorCode::Unauthorized, 401, "MyAI authorization rejected");
    Status cleared = Status::success();
    if (kind == RequestKind::PairingStatus) {
      cleared = clearPendingFailClosed();
    } else if (kind == RequestKind::DeviceCheck ||
               kind == RequestKind::CenterAuthenticated ||
               kind == RequestKind::GatewayBusiness) {
      cleared = clearRuntimeCredentialFailClosed();
    }
    if (!cleared.ok()) return cleared;
    if (kind == RequestKind::PairingStatus ||
...
bool MyAiClient::isPublicGatewayUrl(const std::string& url) {
  ...
  size_t hostBegin = 8;
  size_t hostEnd = url.find_first_of("/:?#", hostBegin);
  const std::string host = lowercase(url.substr(
  ...
  if (host.empty() || host.find('@') != std::string::npos || host == "localhost" ||
      ...
      (host.size() > 4 && host.compare(host.size() - 4, 4, ".lan") == 0) ||
      (host.size() > 9 && host.compare(host.size() - 9, 9, ".internal") == 0) ||
      host.compare(0, 4, "127.") == 0 || host.compare(0, 3, "10.") == 0 ||
      host.compare(0, 8, "192.168.") == 0 ||
      host.compare(0, 8, "169.254.") == 0 || host == "0.0.0.0" || host == "::1")
    return false;
