...
int chineseDigit(const std::string& value) {
  if (value == "零") return 0;
  if (value == "一") return 1;
...
bool parseChineseOrdinal(const std::string& value, int& output) {
  if (value.empty() || value.size() % 3U != 0U) return false;
  int total = 0;
  int digit = -1;
  bool saw = false;
  for (size_t offset = 0; offset < value.size(); offset += 3U) {
    const std::string token = value.substr(offset, 3U);
    const int parsed = chineseDigit(token);
    if (parsed >= 0) {
      digit = parsed;
      saw = true;
      continue;
  ...
  if (!saw || total < 1 || total > 999) return false;
  output = total;
  return true;
}

bool parseEnglishOrdinal(const std::string& value, int& output) {
  std::string digits;
  if (startsWith(value, "image ")) {
    digits = afterPrefix(value, "image ");
    if (startsWith(digits, "number ")) digits = afterPrefix(digits, "number ");
  } else if (value.size() > 6U &&
             value.compare(value.size() - 6U, 6U, " image") == 0) {
    digits = value.substr(0, value.size() - 6U);
    if (digits.size() > 2U) {
      const std::string suffix = digits.substr(digits.size() - 2U);
      if (suffix == "st" || suffix == "nd" || suffix == "rd" || suffix == "th")
        digits.erase(digits.size() - 2U);
    }
  } else {
    return false;
  }
  return parseInt(digits, output) && output >= 1 && output <= 999;
}
...
ParsedCommand LocalCommandParser::parse(const std::string& transcript) const {
  ...
  const bool chineseSpaceQuery = text.size() <= 60 &&
      (text.find("空间") != std::string::npos ||
       text.find("容量") != std::string::npos) &&
      (text.find("剩余") != std::string::npos ||
       text.find("可用") != std::string::npos ||
       text.find("查询") != std::string::npos ||
       text.find("查看") != std::string::npos ||
       text.find("多少") != std::string::npos ||
       text == "存储空间" || text == "储存空间");
  if (chineseSpaceQuery ||
      text == "剩余空间" || text == "查询剩余空间" || text == "查看剩余空间" ||
      text == "可用空间" || text == "查询存储空间" ||
      text == "查询空间" || text == "查看空间" || text == "储存空间" ||
      text == "还有多少空间" || text == "还有多少存储空间" ||
      text == "free space" || text == "storage space" ||
      text == "available space" || text == "query free space" ||
      text == "check free space" || text == "how much space") {
  ...
  if (startsWith(text, "第") && text.size() > std::string("第张").size() &&
    ...
    int ordinal = 0;
    if ((parseInt(digits, ordinal) || parseChineseOrdinal(digits, ordinal)) &&
        ordinal >= 1 && ordinal <= 999) {
      command.kind = CommandKind::SelectImage;
  ...
  int englishOrdinal = 0;
  if (parseEnglishOrdinal(text, englishOrdinal)) {
    command.kind = CommandKind::SelectImage;
    command.targetId = "@" + std::to_string(englishOrdinal);
    command.number = englishOrdinal;
    command.english = true;
    return command;
  }
  ...
  const struct SettingPrefix {
    ...
    bool english;
      ...
      {"图片步数 ", "steps", false}, {"image steps ", "steps", true},
      {"图片模型 ", "model", false}, {"image model ", "model", true},
      {"图片生成提示词 ", "prompt_template", false},
      {"image prompt template ", "prompt_template", true},
  ...
  for (size_t index = 0; index < sizeof(settingPrefixes) / sizeof(settingPrefixes[0]); ++index) {
    ...
    const bool valid = key == "size" ? parseSize(value)
                     : key == "steps" ? (parseInt(value, numeric) &&
                                            numeric >= 1 && numeric <= 50)
