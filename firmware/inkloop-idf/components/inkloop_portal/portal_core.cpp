...
const char* myAiStateName(MyAiPortalState state) {
  switch (state) {
    ...
    case MyAiPortalState::PaymentRequired: return "payment_required";
...
const char* myAiErrorSourceName(MyAiPortalErrorSource source) {
  switch (source) {
    case MyAiPortalErrorSource::None: return "none";
    case MyAiPortalErrorSource::Command: return "command";
    case MyAiPortalErrorSource::Tick: return "tick";
    case MyAiPortalErrorSource::Initialize: return "initialize";
    case MyAiPortalErrorSource::ApplyPrompt: return "apply_prompt";
    case MyAiPortalErrorSource::Pairing: return "pairing";
    case MyAiPortalErrorSource::Authorization: return "authorization";
    case MyAiPortalErrorSource::Aigc: return "aigc";
    case MyAiPortalErrorSource::VoiceConnect: return "voice_connect";
    case MyAiPortalErrorSource::VoiceIngress: return "voice_ingress";
    case MyAiPortalErrorSource::CaptureUpload: return "capture_upload";
    case MyAiPortalErrorSource::Heartbeat: return "heartbeat";
  }
  return "none";
}

const char* myAiErrorCodeName(MyAiPortalErrorCode code) {
  switch (code) {
    case MyAiPortalErrorCode::None: return "none";
    case MyAiPortalErrorCode::InvalidArgument: return "invalid_argument";
    case MyAiPortalErrorCode::InvalidState: return "invalid_state";
    case MyAiPortalErrorCode::Storage: return "storage";
    case MyAiPortalErrorCode::Security: return "security";
    case MyAiPortalErrorCode::Transport: return "transport";
    case MyAiPortalErrorCode::Protocol: return "protocol";
    case MyAiPortalErrorCode::Unauthorized: return "unauthorized";
    case MyAiPortalErrorCode::PaymentRequired: return "payment_required";
    case MyAiPortalErrorCode::RecoveryRequired: return "recovery_required";
    case MyAiPortalErrorCode::PairingExpired: return "pairing_expired";
    case MyAiPortalErrorCode::Conflict: return "conflict";
    case MyAiPortalErrorCode::AppNotRegistered:
      return "app_not_registered";
    case MyAiPortalErrorCode::NoGateway: return "no_gateway";
    case MyAiPortalErrorCode::TooLarge: return "too_large";
    case MyAiPortalErrorCode::Cancelled: return "cancelled";
  }
  return "none";
}

const char* tutorialStepName(TutorialPortalStep step) {
  switch (step) {
    case TutorialPortalStep::PressToTalk: return "press_to_talk";
    case TutorialPortalStep::VoiceLedStates: return "voice_led_states";
    case TutorialPortalStep::GalleryPaging: return "gallery_paging";
    case TutorialPortalStep::DisplayBusyGuard: return "display_busy_guard";
    case TutorialPortalStep::LocalPortal: return "local_portal";
    case TutorialPortalStep::Complete: return "complete";
  }
...
bool validState(const PortalStateSnapshot& state) {
  const bool valid_myai_error = !state.myai_error.available
      ? state.myai_error.source == MyAiPortalErrorSource::None &&
            state.myai_error.code == MyAiPortalErrorCode::None &&
      ...
      : state.myai_error.source != MyAiPortalErrorSource::None &&
            ...
            state.myai_error.code != MyAiPortalErrorCode::None &&
            static_cast<uint8_t>(state.myai_error.code) <=
                static_cast<uint8_t>(MyAiPortalErrorCode::Cancelled) &&
            (state.myai_error.http_status == 0U ||
  ...
  return validText(state.firmware_version, 64U, false, false) &&
         ...
         valid_myai_error &&
         static_cast<uint8_t>(state.tutorial.step) <=
             static_cast<uint8_t>(TutorialPortalStep::Complete) &&
         !(state.tutorial.in_flight &&
           state.tutorial.persistence_pending) &&
         (state.tutorial.step != TutorialPortalStep::Complete ||
          !state.tutorial.in_flight) &&
         ...
         state.settings.image_generation_steps >=
             kMinimumImageGenerationSteps &&
         state.settings.image_generation_steps <=
             kMaximumImageGenerationSteps &&
...
PortalResponse PortalCore::handleAuthenticated(const PortalRequest& request) {
  ...
  if (path == "/api/tutorial/restart") {
    ...
    if (!capabilities.hasDuplexAudio())
      return errorResponse(422, "audio_not_supported");
    return enqueueSimple(request, PortalCommandType::RestartMyAiTutorial);
  }
...
PortalResponse PortalCore::renderState() {
  ...
  json << "]}"
       ...
       << (state.audio_diagnostics.available ? "true" : "false")
       << ",\"lifecycle\":{\"prepared\":"
       << (state.audio_diagnostics.prepared ? "true" : "false")
       << ",\"captureChannelEnabled\":"
       << (state.audio_diagnostics.capture_channel_enabled ? "true"
                                                           : "false")
       << ",\"playbackChannelEnabled\":"
       << (state.audio_diagnostics.playback_channel_enabled ? "true"
                                                            : "false")
       << ",\"prepareAttempts\":"
       << state.audio_diagnostics.prepare_attempts
       << ",\"prepareSuccesses\":"
       << state.audio_diagnostics.prepare_successes
       << ",\"prepareFailures\":"
       << state.audio_diagnostics.prepare_failures
       << ",\"shutdowns\":" << state.audio_diagnostics.shutdowns
       << ",\"shutdownFailures\":"
       << state.audio_diagnostics.shutdown_failures
       << ",\"playbackClockReconfigurations\":"
       << state.audio_diagnostics.playback_clock_reconfigurations
       << ",\"playbackClockReconfigurationFailures\":"
       << state.audio_diagnostics.playback_clock_reconfiguration_failures
       << ",\"sharedPinSelections\":"
       << state.audio_diagnostics.shared_pin_selections
       << ",\"sharedPinSelectionFailures\":"
       << state.audio_diagnostics.shared_pin_selection_failures
       << ",\"preparedPlaybackRateHz\":"
       << state.audio_diagnostics.prepared_playback_rate_hz
       << ",\"lastPrepareError\":"
       << state.audio_diagnostics.last_prepare_error << "}"
       << ",\"capture\":{\"starts\":"
       << state.audio_diagnostics.capture_starts
       << ",\"timeouts\":" << state.audio_diagnostics.capture_timeouts
       << ",\"failures\":" << state.audio_diagnostics.capture_failures
       << ",\"bytes\":" << state.audio_diagnostics.captured_bytes << "}"
       << ",\"playback\":{\"starts\":"
       << state.audio_diagnostics.playback_starts
       << ",\"timeouts\":" << state.audio_diagnostics.playback_timeouts
       << ",\"failures\":" << state.audio_diagnostics.playback_failures
       << ",\"preloadStarts\":"
       << state.audio_diagnostics.playback_preload_starts
       << ",\"forcedAborts\":" << state.audio_diagnostics.forced_aborts
       << ",\"sourceBytes\":"
       << state.audio_diagnostics.played_source_bytes
       << ",\"outputFrames\":"
       << state.audio_diagnostics.played_output_frames
       << ",\"peakPreloadedBytes\":"
       << state.audio_diagnostics.peak_preloaded_bytes << "}"
       << ",\"dma\":{\"callbacks\":"
       << state.audio_diagnostics.playback_dma_callbacks
       << ",\"underruns\":"
       << state.audio_diagnostics.playback_dma_underruns
       << ",\"expectedDrainOverflows\":"
       << state.audio_diagnostics.playback_dma_expected_drain_overflows
       << "},\"feed\":{\"streams\":"
       << state.audio_diagnostics.feed_streams
       << ",\"submitCalls\":" << state.audio_diagnostics.feed_submit_calls
       << ",\"lateSubmits\":" << state.audio_diagnostics.feed_late_submits
       << ",\"estimatedUnderruns\":"
       << state.audio_diagnostics.feed_estimated_underruns
       << ",\"queueClamps\":" << state.audio_diagnostics.feed_queue_clamps
       << ",\"maxSubmitGapUs\":"
       << state.audio_diagnostics.feed_max_submit_gap_us
       << ",\"minimumQueueLeadUs\":"
       << state.audio_diagnostics.feed_minimum_queue_lead_us
       << ",\"maximumQueueLeadUs\":"
       << state.audio_diagnostics.feed_maximum_queue_lead_us
       << ",\"preloadedFrames\":"
       << state.audio_diagnostics.feed_preloaded_frames
       << ",\"submittedFrames\":"
       << state.audio_diagnostics.feed_submitted_frames
       << ",\"consumedFrames\":"
       << state.audio_diagnostics.feed_consumed_frames
       << ",\"estimatedUnderrunFrames\":"
       << state.audio_diagnostics.feed_estimated_underrun_frames
       << ",\"queueOverflowFrames\":"
       << state.audio_diagnostics.feed_queue_overflow_frames
       << ",\"currentQueueFrames\":"
       << state.audio_diagnostics.feed_current_queue_frames
       << ",\"peakQueueFrames\":"
       << state.audio_diagnostics.feed_peak_queue_frames << "}}"
  ...
  json << "]}"
       ...
       << "\",\"lastError\":{\"available\":"
       << (state.myai_error.available ? "true" : "false")
       << ",\"source\":\"" << myAiErrorSourceName(state.myai_error.source)
       << "\",\"code\":\"" << myAiErrorCodeName(state.myai_error.code)
       << "\",\"httpStatus\":" << state.myai_error.http_status
       ...
       << "\",\"inFlight\":"
       << (state.tutorial.in_flight ? "true" : "false")
       << ",\"persistencePending\":"
       << (state.tutorial.persistence_pending ? "true" : "false")
       << ",\"persistenceError\":"
       << (state.tutorial.persistence_error ? "true" : "false")
       << "}},\"settings\":{\"volume\":"
       ...
       << static_cast<unsigned int>(state.settings.image_generation_steps)
       ...
       << ",\"ledRolesSwapped\":"
       << (state.settings.led_roles_swapped ? "true" : "false")
       << "}}}";
...
PortalResponse PortalCore::updateSettings(const PortalRequest& request) {
  ...
  Fields fields;
  const std::vector<std::string> allowed = {
      "volume", "led_brightness", "led_swap", "voice_assistance", "assistant_prompt",
      "image_prompt_template", "image_steps", "negative_prompt", "storage_preference",
  ...
  if (const std::string* value = field(fields, "led_swap")) {
    if (capabilities.rgb_pixels < 2U)
      return errorResponse(422, "led_role_swap_not_supported");
    patch.has_led_roles_swapped = true;
    if (!parseBoolean(*value, patch.led_roles_swapped))
      return errorResponse(422, "invalid_led_swap");
  }
  ...
  if (const std::string* value = field(fields, "image_steps")) {
    std::uint64_t parsed = 0U;
    if (!parseUnsigned(*value, parsed) ||
        parsed < kMinimumImageGenerationSteps ||
        parsed > kMaximumImageGenerationSteps) {
      return errorResponse(422, "invalid_image_steps");
    }
    patch.has_image_generation_steps = true;
    patch.image_generation_steps = static_cast<std::uint8_t>(parsed);
  }
...
</style></head><body><main><h1>Inkloop 本地管理</h1><p class="muted">操作会安全排队到设备任务。</p>
...
const char* portalCommandTypeName(PortalCommandType value) {
  switch (value) {
    ...
    case PortalCommandType::RestartMyAiTutorial:
      return "RESTART_MYAI_TUTORIAL";
